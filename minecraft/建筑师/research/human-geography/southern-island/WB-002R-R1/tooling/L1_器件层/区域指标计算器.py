...
def load_observed(path):
    ...
    rows=np.fromiter((v for row in db.execute('SELECT * FROM samples ORDER BY z,x') for v in row),dtype=np.int32).reshape(-1,len(names))
    shape=(BOUNDS[3]-BOUNDS[1]+1,BOUNDS[2]-BOUNDS[0]+1)
    assert len(rows)==shape[0]*shape[1]
    result={name:np.ascontiguousarray(rows[:,i].reshape(shape)) for i,name in enumerate(names)}
...
def compute(a):
    ...
    # 全范围固定高程代理不等于山地分类；海岸与高地内部仍由坡度、起伏区分。
    result['local_highland_proxy']=(land&(h>=100)).astype(np.int8)
...
def numerical_zoning(a,m,island):
    ...
    f=np.array(features);scale=np.array([32,16,0.5,0.25,0.05,0.5]+[1]*len(bids));v=(f-f.mean(axis=0))/scale
    results=[];maps={}
    for k in range(2,6):
        best=None
        for seed in (2,17,83):
