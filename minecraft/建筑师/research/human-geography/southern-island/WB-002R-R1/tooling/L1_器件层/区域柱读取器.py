...
import numpy as np
import nbtlib
from L0_公理层.区域契约 import AIR,WATER,ICE,vegetation,artificial,plant_category
...
class Reader:
    ...
    def raw(self,cx,cz):
        ...
        if int(d['DataVersion'])!=4903 or str(d['Status'])!='minecraft:full' or (int(d['xPos']),int(d['zPos']))!=(cx,cz):raise ValueError(f'区块前提不满足: requested={cx},{cz}; version={d["DataVersion"]}; status={d["Status"]}; stored={d["xPos"]},{d["zPos"]}')
        return d,path,loc>>8
    def read(self,cx,cz):
        key=(cx,cz)
        ...
        # 4格水平格点逐Y统计过滤地表以上的真实palette；不把log自动解释为枯树。
        sample=(xs%4==0)&(zs%4==0);vegcounts={k:np.zeros(256,dtype=int) for k in ('leaf','trunk','dry_ground_vegetation','other_vegetation')}
        names_count={}
        for y in range(int(gy[sample].min())+1,int(sy[sample].max())+1):
            active=sample&(y>gy)&(y<=sy)
            if not active.any():continue
            values=states_at(np.full(256,y),active)
            for i in np.flatnonzero(active):
                name=self.state_values[values[i]]['Name'];cat=plant_category(name)
                if cat in vegcounts:vegcounts[cat][i]+=1
                if cat not in ('air','other'):names_count[name]=names_count.get(name,0)+1
        self.db.executemany('INSERT INTO vegetation VALUES (?,?,?,?,?,?)',[(int(xs[i]+cx*16),int(zs[i]+cz*16),*(int(vegcounts[k][i]) for k in vegcounts)) for i in np.flatnonzero(sample)])
        self.db.executemany('INSERT INTO vegetation_palette VALUES (?,?,?,?)',[(cx,cz,n,c) for n,c in sorted(names_count.items())])
        filtered_biome=np.zeros(256,dtype=np.int32)
        for sec in np.unique(gy//16):
            select=gy//16==sec;p=sections[int(sec)]['biomes'];pal=np.array([self.biome_id(v) for v in p['palette']])
            values=pal[unpack(p.get('data'),max(1,(len(pal)-1).bit_length()),64)]
            indices=((gy[select]%16)//4)*16+(zs[select]//4)*4+xs[select]//4;filtered_biome[select]=values[indices]
        ...
        rows=np.column_stack((xs+cx*16,zs+cz*16,sy,gy,floor,wy,bottom,top,support,np.where(wet,ws,air),biome,covered.astype(int)&wet,suspicious,filtered_biome)).astype(np.int32)
...
def connect(path):
    ...
    CREATE TABLE IF NOT EXISTS samples(x INTEGER,z INTEGER,surface_y INTEGER,exposed_y INTEGER,ocean_floor_y INTEGER,water_y INTEGER,water_bottom INTEGER,surface_state INTEGER,exposed_state INTEGER,water_state INTEGER,biome INTEGER,ice_covered INTEGER,artificial_material INTEGER,filtered_biome INTEGER,PRIMARY KEY(z,x)) WITHOUT ROWID;
    CREATE TABLE IF NOT EXISTS vegetation(x INTEGER,z INTEGER,leaf INTEGER,trunk INTEGER,dry_ground_vegetation INTEGER,other_vegetation INTEGER,PRIMARY KEY(z,x)) WITHOUT ROWID;
    CREATE TABLE IF NOT EXISTS vegetation_palette(cx INTEGER,cz INTEGER,name TEXT,count INTEGER,PRIMARY KEY(cx,cz,name)) WITHOUT ROWID;
