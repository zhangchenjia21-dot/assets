...
BOUNDS=tuple(json.loads((ROOT/'profile/scope-completion.json').read_text(encoding='utf-8'))['bounds'])
...
def main(world):
    ...
    m=np.load(ROOT/'raw-or-queryable/derived.npz');main=m['whole_group'];rng=np.random.default_rng(2002);points={}
    ...
    add(main,'group_random',45);add(main&(m['relief32']>=8),'higher_relief');add(main&(m['terrain_class']==1),'lowland');add(m['shoreline'],'shore');add(m['west'],'western_reuse');add((m['water_component']>1),'internal_or_small_water');add(m['east'],'eastern_full',120)
    expanded=np.indices(main.shape)[1]+BOUNDS[0]>415
    add(expanded,'new_east_area',100)
    for x,z in db.execute('SELECT x,z FROM samples WHERE water_y=-32768 ORDER BY exposed_y DESC,z,x LIMIT 4'):points[(x,z)]='highest_current_land'
    ez,ex=np.where(m['east'])
    for index in (int(np.argmin(ex)),int(np.argmax(ex)),int(np.argmin(ez)),int(np.argmax(ez))):points[(int(ex[index]+BOUNDS[0]),int(ez[index]+BOUNDS[1]))]='east_outline_extreme'
