...
/** 作业容器：去重调色板 + 两种操作（单块 / 长方体） */
export class 作业 {
  constructor() { this.索引 = new Map(); this.palette = []; this.operations = []; }
  配(方块) {
  ...
  /** 单块 */
  放(x, y, z, 方块) { this.operations.push([x, y, z, this.配(方块)]); }
  /** 长方体（自动归一化角点） */
  盒(x1, y1, z1, x2, y2, z2, 方块) {
    this.operations.push([Math.min(x1, x2), Math.min(y1, y2), Math.min(z1, z2), Math.max(x1, x2), Math.max(y1, y2), Math.max(z1, z2), this.配(方块)]);
  }
